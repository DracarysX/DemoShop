...
@app.get("/product-similarity", response_class=HTMLResponse)
async def get_product_similarity(threshold: float = 0.1):
    ...
    for event in analytics_events:
        ...
        # Engagement score: clicks=2 points, view_time=0.01 points per ms
        if event["eventType"] == "click":
            engagement_matrix[product_idx, user_idx] += 2.0
        elif event["eventType"] == "view":
            view_duration = event.get("viewDuration", 0)
            engagement_matrix[product_idx, user_idx] += view_duration * 0.01
    ...
    # Convert to HTML
    graph_html = fig.to_html(include_plotlyjs='cdn', div_id='graph', config={'displayModeBar': False})
